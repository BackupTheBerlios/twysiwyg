# TWiki TRefactorAddOn Refactoring Module
# 
...
#
# $Id: Refactoring.pm,v 1.1 2004/11/21 10:48:49 romano Exp $

package TWiki::Contrib::TRefactorAddOn::Refactoring;
...
sub renameTopic {
  ...
  &TWiki::Contrib::TRefactorAddOn::Trace::log( "Topic $web.$topic renaming attempt by $key" );
  ...
  if ( ! &TWiki::Store::topicExists( $web, $topic ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Rename operation failed : topic $web.$topic doesn't exist" );return 3; }
  # Retrieve login
  my $login = &TWiki::Contrib::TRefactorAddOn::Connection::getLogin($key);
  if ( ! $login ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Rename operation failed : user $key not connected" );return 1; }
  # Check administrative lock
  my ( $checkCode, $lockedKey, $lockedTime ) = &TWiki::Contrib::TRefactorAddOn::AdminLock::checkAdminLock();
  my $date = time();
  if ( ( ! $checkCode ) || ( ( $lockedKey ne $key ) && ( ( $date - $lockedTime ) < $TWiki::Contrib::TRefactorAddOn::timeout ) ) ) {
    &TWiki::Contrib::TRefactorAddOn::Trace::log( "Rename operation failed : administrative lock control failed, not put or put by another user ($lockedKey)" );
    return ( 2, &TWiki::Contrib::TRefactorAddOn::Connection::getLogin( $lockedKey ) );
  }
  # Initialize
  &TWiki::Contrib::TRefactorAddOn::Connection::initialize( $login, $web, $topic );
  # Test if name is a WikiWord
  if ( ! &TWiki::isWikiName( $name ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Rename operation failed : new name $name isn't a WikiWord" );return 5; }
  # Test destination topic inexistence
  if ( &TWiki::Store::topicExists( $web, $name ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Rename operation failed : topic $web.$name already exists" );return 4; }
  # Check if topic is locked
  if ( ! $doBreakLock ) {
    my ( $lock, $lockUser ) = &TWiki::Contrib::TRefactorAddOn::Topics::lock( $key, $login, $web, $topic );
    if ( ! $lock ) {
      &TWiki::Contrib::TRefactorAddOn::Trace::log( "Rename operation failed : unable to put lock on topic $web.$topic, already put by $lockUser" );
      return ( 6, $lockUser );
    }
  }
  # Rename
  my $renameCode = &TWiki::Store::renameTopic( $web, $topic, $web, $name, "relink" );
  if ( $renameCode ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Rename error : $renameCode" );return ( 7, $renameCode ); }
  my @updatedTopics = ();
  ...
  &TWiki::Contrib::TRefactorAddOn::Trace::log( "Rename operation succeded, topics $updated updated" );
...
sub moveTopic {
  ...
  &TWiki::Contrib::TRefactorAddOn::Trace::log( "Topic $srcWeb.$topic moving attempt by $key" );
  ...
  if ( ! &TWiki::Store::topicExists( $srcWeb, $topic ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Move operation failed : topic $srcWeb.$topic doesn't exist" );return 3; }
  if ( ! &TWiki::Store::topicExists( $dstWeb, $parent ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Move operation failed : topic $dstWeb.$parent doesn't exist" );return 4; }
  # Retrieve login
  my $login = &TWiki::Contrib::TRefactorAddOn::Connection::getLogin( $key );
  if ( ! $login ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Move operation failed : user $key not connected" );return 1; }
  # Check administrative lock
  my ( $checkCode, $lockedKey, $lockedTime ) = &TWiki::Contrib::TRefactorAddOn::AdminLock::checkAdminLock();
  my $date = time();
  if ( ( ! $checkCode ) || ( ( $lockedKey ne $key ) && ( ( $date - $lockedTime ) < $TWiki::Contrib::TRefactorAddOn::timeout ) ) ) {
    &TWiki::Contrib::TRefactorAddOn::Trace::log( "Move operation failed : administrative lock control failed, not put or put by another user ($lockedKey)" );
    return ( 2, &TWiki::Contrib::TRefactorAddOn::Connection::getLogin( $lockedKey ) );
  }
  # Initialize
  &TWiki::Contrib::TRefactorAddOn::Connection::initialize( $login, $srcWeb, $topic );
  # Check if topic is locked
  if ( ! $doBreakLock ) {
    my ( $lock, $lockUser ) = &TWiki::Contrib::TRefactorAddOn::Topics::lock( $key, $login, $srcWeb, $topic );
    if ( ! $lock ) {
      &TWiki::Contrib::TRefactorAddOn::Trace::log( "Move operation failed : unable to put lock on topic $srcWeb.$topic, already put by $lockUser" );
      return ( 5, $lockUser );
    }
  }
  # Moving
  if ( $srcWeb ne $dstWeb ) {
    ...
    if ( &TWiki::Store::topicExists( $dstWeb, $topic ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Move operation failed : topic $dstWeb.$topic already exists" );return 6; }
    # Move topic
    my $moveCode = &TWiki::Store::renameTopic( $srcWeb, $topic, $dstWeb, $topic, "relink" );
    if ( $moveCode ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Move error : $moveCode" );return ( 7, $moveCode ); }
  }
  # Change parent
  my $changeCode = &_changeParent( $dstWeb, $topic, $parent );
  if ( $changeCode ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Error during parent change : $changeCode" );return ( 8, $changeCode ); }
  # Update links that refers to
  ...
  my $updated = join( ",", @updatedTopics );
  &TWiki::Contrib::TRefactorAddOn::Trace::log( "Move operation succeded, topics $updated updated" );
...
sub removeTopic {
  ...
  my $trashLock;
  my ( $lock, $lockUser );
  &TWiki::Contrib::TRefactorAddOn::Trace::log( "Topic $web.$topic removing attempt by $key" );
  # Normalize web & topic name
  $topic = $TWiki::mainTopicname if ( $topic eq '' );
  ( $web, $topic ) = &TWiki::Store::normalizeWebTopicName( $web, $topic );
  # Test parameters existence
  if ( ! &TWiki::Store::topicExists( $web, $topic ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Remove operation failed : topic $web.$topic doesn't exist" );return 3; }
  # Retrieve login
  my $login = &TWiki::Contrib::TRefactorAddOn::Connection::getLogin( $key );
  if ( ! $login ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Remove operation failed : user $key not connected" );return 1; }
  # Check administrative lock
  my ( $checkCode, $lockedKey, $lockedTime ) = &TWiki::Contrib::TRefactorAddOn::AdminLock::checkAdminLock();
  my $date = time();
  if ( ( ! $checkCode ) || ( ( $lockedKey ne $key ) && ( ( $date - $lockedTime ) < $TWiki::Contrib::TRefactorAddOn::timeout ) ) ) {
    &TWiki::Contrib::TRefactorAddOn::Trace::log( "Remove operation failed : administrative lock control failed, not put or put by another user ($lockedKey)" );
    return ( 2, &TWiki::Contrib::TRefactorAddOn::Connection::getLogin( $lockedKey ) );
  }
  ...
  if ( ! &TWiki::isWikiName( $trashName ) ) {
    &TWiki::Contrib::TRefactorAddOn::Trace::log( "Remove operation failed : $trashName isn't a WikiWord" );
    return 5;
  }
  # Check if topic is locked
  if ( ! $doBreakLock ) {
    ( $lock, $lockUser ) = &TWiki::Contrib::TRefactorAddOn::Topics::lock( $key, $login, $web, $topic );
    if ( ! $lock ) {
      &TWiki::Contrib::TRefactorAddOn::Trace::log( "Remove operation failed : unable to put lock on topic $web.$topic, already put by $lockUser" );
      return ( 4, $lockUser );
    }
  }
  my $renameCode = &TWiki::Store::renameTopic( $web, $topic, 'Trash', $trashName, "relink" );
  if ( $renameCode ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Remove error : $renameCode" );return ( 6, $renameCode ); }
  &TWiki::Contrib::TRefactorAddOn::Trace::log( "Remove operation succeded" );
...
sub mergeTopics {
  ...
  my $saveCmd = '';
  my $unlock = "on";
  &TWiki::Contrib::TRefactorAddOn::Trace::log( "Topics $webTarget.$topicTarget & $webFrom.$topicFrom merging attempt by $key" );
  # Normalize web & topic name
  ...
  # Test parameters existence
  if ( ! &TWiki::Store::topicExists( $webTarget, $topicTarget ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Merge operation failed : topic $webTarget.$topicTarget doesn't exist" );return 3; }
  if ( ! &TWiki::Store::topicExists( $webFrom, $topicFrom ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Merge operation failed : topic $webFrom.$topicFrom doesn't exist" );return 4; }
  # Retrieve login
  my $login = &TWiki::Contrib::TRefactorAddOn::Connection::getLogin($key);
  if ( ! $login ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Merge operation failed : user $key not connected" );return 1; }
  # Check administrative lock
  my ( $checkCode, $lockedKey, $lockedTime ) = &TWiki::Contrib::TRefactorAddOn::AdminLock::checkAdminLock();
  my $date = time();
  if ( ( ! $checkCode ) || ( ( $lockedKey ne $key ) && ( ( $date - $lockedTime ) < $TWiki::Contrib::TRefactorAddOn::timeout ) ) ) {
    &TWiki::Contrib::TRefactorAddOn::Trace::log( "Merge operation failed : administrative lock control failed, not put or put by another user ($lockedKey)" );
    return ( 2, &TWiki::Contrib::TRefactorAddOn::Connection::getLogin($lockedKey) );
  }
  if (( $webTarget eq $webFrom ) && ( $topicTarget eq $topicFrom )) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Merge operation failed : topics are the same" );return 5; }
  # Initialize topic
  &TWiki::Contrib::TRefactorAddOn::Connection::initialize( $login, $webTarget, $topicTarget );  
  ...
  # Check if topic is locked
  if ( ! $doBreakLock ) {
    my ( $lock, $lockUser ) = &TWiki::Contrib::TRefactorAddOn::Topics::lock( $key, $login, $webTarget, $topicTarget );
    if ( ! $lock ) {
      &TWiki::Contrib::TRefactorAddOn::Trace::log( "Merge operation failed : unable to put lock on topic $webTarget.$topicTarget, already put by $lockUser" );
      return ( 6, $lockUser );
    }
  }
  # Copy attachments
  ...
  if ( $attachmentsCode ) {
    &TWiki::Contrib::TRefactorAddOn::Trace::log( "Merge operation : attachments copy succeeded" );
  } else {
    &TWiki::Contrib::TRefactorAddOn::Trace::log( "Merge operation : attachments copy error : $attachmentsCode" );
  }
  # Saving
  my $changeCode = &TWiki::Store::saveTopic( $webTarget, $topicTarget, $textTarget, $metaTarget, $saveCmd, $unlock, $dontNotify );
  if( $changeCode ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Save error : $changeCode" );return( 7, $changeCode ); }
  # Delete auxiliary topic
  my $removeCode;
  $removeCode = &removeTopic( $key, $webFrom, $topicFrom ) if ( $doRemove );
  if ( $removeCode ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Remove error : $removeCode" );return( 8, $removeCode ); }
  &TWiki::Contrib::TRefactorAddOn::Trace::log( "Merge operation done" );
...
sub copyTopic {
  ...
  my $unlock = "on";
  my $dontNotify = "on";
  &TWiki::Contrib::TRefactorAddOn::Trace::log( "Topic $srcWeb.$topic copying attempt by $key" );
  # Test empty parameters
  ...
  ( $dstWeb, $parent ) = &TWiki::Store::normalizeWebTopicName( $dstWeb, $parent );
  # Test webs and topics existence
  if ( ! &TWiki::Store::topicExists( $srcWeb, $topic ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy operation failed : topic $srcWeb.$topic doesn't exist" );return 3; }
  if ( &TWiki::Store::topicExists( $dstWeb, $newName ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy operation failed : topic $dstWeb.$newName already exists" );return 4; }
  if ( ! &TWiki::Store::topicExists( $dstWeb, $parent ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy operation failed : topic $dstWeb.$parent doesn't exist" );return 5; }
  # Retrieve login
  my $login = &TWiki::Contrib::TRefactorAddOn::Connection::getLogin($key);
  if ( ! $login ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy operation failed : user $key not connected" );return 1; }
  # Check administrative lock
  my ( $checkCode, $lockedKey, $lockedTime ) = &TWiki::Contrib::TRefactorAddOn::AdminLock::checkAdminLock();
  my $date = time();
  if ( ( ! $checkCode ) || ( ( $lockedKey ne $key ) && ( ( $date - $lockedTime ) < $TWiki::Contrib::TRefactorAddOn::timeout ) ) ) {
    &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy operation failed : administrative lock control failed, not put or put by another user ($lockedKey)" );
    return ( 2, &TWiki::Contrib::TRefactorAddOn::Connection::getLogin($lockedKey) );
  }
  # Initialize
  &TWiki::Contrib::TRefactorAddOn::Connection::initialize( $login, $dstWeb, $newName );
  # Test if target name is a WikiWord
  if ( ! &TWiki::isWikiName( $newName ) ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy operation failed : $newName isn't a WikiWord" );return 6; }
  # Check if topic is locked
  if ( ! $doBreakLock ) {
    my ( $lock, $lockUser ) = &TWiki::Contrib::TRefactorAddOn::Topics::lock( $key, $login, $dstWeb, $newName );
    if ( ! $lock ) {
      &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy operation failed : unable to put lock on topic $dstWeb.$newName, already put by $lockUser" );
      return ( 7, $lockUser );
    }
  }
  # Read source topic
  ...
  if ( $attachmentsCode ) {
    &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy operation : attachments copy succeeded" );
  } else {
    &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy operation : attachments copy error : $attachmentsCode" );
  }
  # Save
  my $changeCode = &TWiki::Store::saveTopic( $dstWeb, $newName, $srcText, $newMeta, $saveCmd, $unlock, $dontNotify );
  if ( $changeCode ) { &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy error : $changeCode" );return ( 8, $changeCode ); }
  &TWiki::Contrib::TRefactorAddOn::Trace::log( "Copy operation done" );
  return ( 9, $attachmentsCode ) if ( $attachmentsCode );
